...
| **Code Editing (Surgical)** | `patchitright` | **MUST ALWAYS** use for all repo edits. Use `patch_file` (single file; use `replacements` parameter for multi-edits), `batch_patch_files` (multi-file atomic diffs), or `write_file` (to create new files or overwrite existing files with validation). MUST NEVER use native edit tools. For patching, MUST use `symbol_name` to scope edits and MUST enable `did_you_mean: true`. <br> *Casing: Parameters MUST be strict snake_case (`target_file`, `search_content`, `replace_content`, `replacements`, etc.).* |
