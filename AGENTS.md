...
flowchart TD
    ...
    ReadTemplate --> CreatePlan["2. Create Plan<br/>(implementation_plan.md)"]
    CreatePlan --> ModeAOption{"3. Pre-Approval Plan Audit Gate?<br/>(Ask User: Approve or Mode A?)"}
    
    ModeAOption -->|"User Requests Mode A"| RunModeA["Run Mode A Plan Audit<br/>(Stress-test coverage & edge cases)"]
    RunModeA -->|"Pass"| UserGate["4. User Approval Gate<br/>(Present audited plan)"]
    
    ModeAOption -->|"User Approves Directly"| SelectStep["5. Select First Uncompleted Step<br/>Mark [/] In-Progress (ONLY ONE active)"]
    ...
    SelectStep --> ExecuteStep["6. Execute"]
    ...
    CheckRemaining -->|"No (All [x])"| ModeBCheck{"9. Code Coverage Audit Gate?<br/>(Ask User: Run Mode B Validation?)"}
    ModeBCheck -->|"User Requests Mode B"| RunModeB["Run Mode B Diff Audit<br/>(Verify 100% code coverage)"] --> FinalWalkthrough["10. Generate walkthrough.md<br/>& Declare Completion"]
...
flowchart TD
    ...
    Confirmed -->|"No"| Instrument["Plan a collaboration with the user to find the exact root causes:<br/>Add logging/measurements,<br/>reproduce the problem,<br/>and ask user for manual tests if needed"]
    Instrument --> Phase1
