...
| **Code Quality & CI/CD** | Analyzing pull requests, resolving sonar code smells, remediating bugs, or fixing CI/CD pipeline issues. | `sonarqube-workflow`, `sonar-remediation`, `sonar-remediation-scripts`, `sonarcloud-ci-workflow` |
| **Productivity & Management** | Writing PR descriptions, managing custom skills, triaging issues, handoff to other agents, requirements gathering, or creating tickets. | `write-pr`, `create-and-update-pr`, `write-for-ai`, `manage-custom-skills`, `manage-global-policies`, `to-prd`, `to-issues`, `triage`, `review`, `handoff`, `grill-me`, `grill-with-docs`, `caveman`, `ponytail`, `ponytail-audit`, `ponytail-debt`, `ponytail-gain`, `ponytail-help`, `ponytail-review`, `update-mcp`, `review-upstream` |
...
| **Code Editing (Surgical)** | `patchitright` | **MUST ALWAYS** use for all repo edits. Use `patch_file` (single file; use `replacements` parameter for multi-edits), `batch_patch_files` (multi-file atomic diffs), or `write_file` (to create new files or overwrite existing files with validation). MUST NEVER use native edit tools. For patching, MUST use `symbol_name` to scope edits. When replacing functions or classes, prefer using `symbol_scope: "body"` or `"full"` without `search_content` to drastically reduce token overhead and avoid offset/indentation drift. Fall back to classic `search_content` only when editing a specific sub-block inside a function, and always enable `did_you_mean: true`. <br> *Casing: Parameters MUST be strict snake_case (`target_file`, `search_content`, `replace_content`, `replacements`, etc.).* |
