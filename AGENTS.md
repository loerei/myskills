...
*   **Architecture & Refactoring Alerts:** Before, during, or after executing a task, if you identify that the codebase architecture is not optimized for modifications (e.g., a change requires editing multiple highly coupled files, modifying duplicate logic blocks, or if implementing the change risks introducing side-effects like breaking desktop code when adding mobile support), you **MUST** immediately read `/improve-codebase-architecture` and propose an architectural improvement plan to the user before writing implementation code.
