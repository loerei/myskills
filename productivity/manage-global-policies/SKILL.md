...
### 3. Verify Policy Skill Coverage & Sync
Run the automated CLI coverage audit tool to ensure 100% of skills are documented and zero orphan skills exist across policies:
```powershell
agents audit
```
- To auto-insert missing skills into policy tables:
```powershell
agents audit --add
```
- To auto-prune orphan/obsolete skills from policy tables:
```powershell
agents audit --prune
```
- For full 2-way sync:
