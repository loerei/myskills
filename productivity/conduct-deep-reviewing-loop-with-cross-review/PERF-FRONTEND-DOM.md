# Frontend DOM Virtualization and Rendering Budgets
...
- [ ] Layout Thrashing Avoidance: Ensure read-write operations targeting the DOM are batched together. Reject interleaved layout reads (`element.offsetHeight`) and writes (`element.style.height`).
...
- Forced Synchronous Reflow: Avoid repetitive DOM geometry recalculations; isolate DOM mutations and batch read/write cycles using requestAnimationFrame or ResizeObserver to eliminate browser engine reflow penalties.
