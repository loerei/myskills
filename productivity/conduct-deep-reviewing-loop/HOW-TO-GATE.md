...
11. **Feature Flag & Telemetry Lifecycle Governance**: Reject reviewer feedback that demands feature flags without lifecycle bounds (owner, 30-day ISO-8601 death-clock, deletion test plan) or un-guarded telemetry logging in high-frequency loops (>1,000 ops/sec). Permanent kill-switches are restricted to high-blast-radius external integrations and circuit breakers.
12. **Configuration Modernization over Indefinite Toleration**: Reject reviewer feedback that demands dual-format configuration loaders, backward-compatibility wrapper shims, or permissive schemas on internal execution paths. Enforce that compatibility with existing test suites and deployment baselines must be achieved via Kent Beck's preparatory structural sequencing ($S \to B$), requiring the DA to stage the modernization of configuration files, environment contracts, and test fixtures as an explicit prerequisite task ($S$) before feature logic ($B$) is introduced.
...
| **UX/UI Redundancy & Emoji Anti-Pattern** | UI element adds user friction, duplicates existing component, breaks consistency, or specifies raw emoji characters instead of vector SVG icons. | **ACCEPT**: Simplify/remove redundant elements and enforce clean vector SVG icon requirements in target DA during Host DA mutation. |
...
| **Defensive Fallback Bloat (Internal Core)** | Reviewer proposes cascading fallbacks (`?? default ?? backup`), synthetic default values, or heuristic property sniffing on internal core domain paths. | **GATE FOR REFINEMENT / REMOVAL**: Reject internal fallback bloat. Postel's Law applies to external ingress only. Demand reviewer reframe remediation to enforce strict fail-fast contracts and boundary parsing. |
...
| **Ad-Hoc Configuration & Runtime Shim Bloat** | Reviewer proposes solving configuration or environment changes by demanding runtime dual-format loaders, ad-hoc wrapper adapters, fallback environment property sniffing, or permissive schema flags (e.g. Zod `.passthrough()`, Pydantic `extra = 'allow'`). | **GATE FOR REFINEMENT**: Reject runtime fallback shims under the Dual Optimization Invariant. Demand reviewer reframe remediation to require a strictly validated canonical configuration parser and staged preparatory modernization of configuration files and test fixtures ($S$) directly inside target DA before feature logic ($B$). |
...
4. **Resolve GUI Layout vs Accessibility Conflicts (and Loading States)**: When visual layout stability (CLS), loading indicators, or empty accessibility landmarks conflict:
   ...
   - Enforce vector SVG icons over raw emojis across all UI controls and specification mockups to ensure theme token binding (`currentColor`) and cross-OS rendering consistency.
...
7. **Resolve Fault Resilience vs. Anti-Defensive-Bloat (The Fail-Fast Invariant)**: When a reviewer demands cascading fallbacks or synthetic defaults on internal domain calls under the guise of resilience, Host MUST NOT allow these fallbacks into the DA. Resilience at external boundaries is achieved via schema parsing and explicit error models; internal domain logic MUST fail fast when contracts are breached.
8. **Resolve Operational Safety vs. Codebase Debt (The Feature Toggle Invariant)**: When a reviewer demands feature flags to guard low-risk internal changes without an owner, death-clock, or deletion plan, Host MUST reject the flag. Operational toggles are liabilities that decay code readability; feature safety for internal logic must be established via automated test coverage rather than runtime flag branching.
9. **Resolve Backward Compatibility vs. Zero Debt Accumulation (The Configuration Modernization Invariant)**: When a reviewer or DA attempts to preserve compatibility with outdated configuration files, legacy environment variables, or stale test fixtures by introducing runtime branching, dual-format parsers, or ambient fallbacks, Host MUST NOT allow these mechanisms into the DA. Host enforces that compatibility with existing test baselines must be achieved by preparatory structural modernization ($S$), updating configuration files, environment definitions, and test fixtures first, directly within the DA, keeping domain loaders strictly canonical.
