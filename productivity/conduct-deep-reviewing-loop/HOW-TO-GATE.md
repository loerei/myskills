...
| **Pedantic / Stylistic Preference** | Requests rephrasing, renaming, or cosmetic adjustments without functional impact. | **GATE FOR REMOVAL**: Demand reviewer removal or mark as non-blocking. Reviewer removes from blocking issues in `<Role>.md`. |
...
4. **Resolve GUI Layout vs Accessibility Conflicts**: When visual layout stability (CLS) conflicts with empty accessibility landmarks (e.g. empty `role="toolbar"`):
   - Enforce the 3-tier precedence: (1) `Context.md` explicit user directives, (2) existing codebase conventions, (3) Default: smooth animated accordion transitions (CSS grid `grid-template-rows: 0fr -> 1fr`, `opacity`, and easing) rather than static persistent slots or sudden non-animated collapse, strictly preserving error recovery controls in `catch` blocks.
