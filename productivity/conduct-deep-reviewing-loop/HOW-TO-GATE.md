...
13. **Platform Primitives & Event Transparency over Imperative Hacks**: Reject reviewer feedback demanding custom JavaScript timers, manual focus-trap scripts, outside-click handlers, or `stopPropagation()` where browser platform standards exist (HTML `<dialog>`, Popover API, `inert` attribute, CSS `@starting-style`, `scrollbar-gutter: stable`, preventable event contracts).
...
| **Ad-Hoc Z-Index Escalation** | Specification or remediation prescribes arbitrary numeric z-indices (e.g. `z-index: 500`, `9999`) or fails to leverage browser Top Layer (`<dialog>`, Popover API) for promoted overlays. | **GATE FOR REFINEMENT**: Reject arbitrary stacking numbers. Demand reviewer mandate design system elevation tokens (e.g. `--zIndex-sticky`) and browser Top Layer primitives. |
...
| **Imperative UI Shims** | Specification prescribes manual JavaScript timers (`setTimeout`, `setInterval`) for CSS animation sync, manual `inert` DOM tree-walking, or script-based outside-click dismiss listeners. | **GATE FOR REFINEMENT**: Reject imperative shims. Demand reviewer specify native platform standards: CSS `@starting-style`, `transition-behavior: allow-discrete`, Popover API light-dismiss, or HTML `inert`. |
