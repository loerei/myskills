...
- **Canonical Icon-Only with Accessible Tooltip Invariant**: In space-constrained layouts (e.g. toolbars, table row actions), prioritize canonical icon-only controls for unambiguous, universally recognized actions (e.g. settings gear, trash delete). Icon-only controls MUST include an accessible label (`aria-label` or `aria-labelledby`) and a visual tooltip rendered on hover and keyboard focus. Non-canonical, domain-specific, or destructive actions without confirmation MUST retain text labels to prevent mystery meat navigation.
