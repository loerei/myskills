...
- **Stacking Context & Top Layer**: When specifying overlays, tooltips, flyouts, and modals, you MUST mandate browser Top Layer primitives (HTML `<dialog showModal()>` for modal dialogs; HTML Popover API `popover="auto|manual"` for non-modal flyouts) to bypass document stacking contexts natively. For in-document positioned layers (e.g. sticky headers, floating indicators), you MUST mandate the target design system's tokenized elevation/z-index scale (e.g. `--zIndex-sticky`, `--zIndex-dropdown`). You MUST strictly BAN arbitrary integer z-indices (magic numbers). For containers enclosing complex layered children, mandate `isolation: isolate` to contain local stacking contexts.
- **Atomic Acceptance Criteria**: You MUST audit interface specifications for contract granularity. You MUST strictly BAN monolithic acceptance criteria that aggregate multiple interactive states, responsive behaviors, keyboard navigation flows, and visual edge cases into compound sentences or paragraphs exceeding 300 characters. Every interactive state, responsive breakpoint shift, keyboard navigation path, and error condition MUST be specified as a distinct, single-responsibility Acceptance Criterion (in Given-When-Then format or a structured state matrix). If a specification contains monster criteria, you MUST return `STATUS: REVISIONS NEEDED` demanding atomic decomposition.
- **Platform Primitives over Shims**: You MUST strictly BAN imperative JavaScript timers (`setTimeout`, `setInterval`) specified to coordinate visual transitions, synchronize visibility states, or delay DOM unmounting. All entry and exit animations for dynamic containers and overlays MUST be specified declaratively using native CSS transitions (`@starting-style`, `transition-behavior: allow-discrete`) or CSS animations. Container dismissals MUST mandate native Popover API light-dismiss mechanics. Background element isolation MUST mandate the HTML `inert` attribute, strictly banning recursive manual `tabindex="-1"` and `aria-hidden` scripts.
