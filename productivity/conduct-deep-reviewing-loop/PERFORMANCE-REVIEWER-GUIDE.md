...
- **Throughput Latency vs Staleness Timeouts**: When auditing or prescribing timeouts on streaming I/O, chunked processing, or long-running pipelines, you MUST NOT penalize throughput latency with arbitrary total wall-clock timers that kill active, forward-progressing jobs. You MUST specify rolling inactivity/idle timeouts (triggered only by zero delta over an inactivity threshold) to distinguish healthy slow progress from deadlocks, hung sockets, or pipeline stalls.
