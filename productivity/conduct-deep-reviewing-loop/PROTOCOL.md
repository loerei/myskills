...
1. Host summons active roles for the current tier batch in a single wave (simultaneously dispatching parallel roles via a single `invoke_subagent` call), initializes `PendingTierRoles` containing all active roles in the tier, initializes per-role probe counter `ProbeCount[role] = 0`, and arms a 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on reviewers liveness", TimerCondition="any")`. Upon receiving an asynchronous reactive wakeup message from a reviewer, if `.scratch/deep-review/reports/<Role>.md` exists on disk, Host removes that reviewer from `PendingTierRoles`; if the report does not exist on disk but the message is an active confirmation, Host resets `ProbeCount[role] = 0` and continues waiting. If `PendingTierRoles` is non-empty, Host re-arms the 180s liveness check timer, does NOT inspect reports on disk or advance to gating evaluation, and ends its turn to continue waiting. If the 180s liveness check timer expires while `PendingTierRoles` is non-empty, Host calls `manage_subagents(Action="list")`: if a reviewer is `running`, Host MUST NOT send any message (re-arming the timer to let it continue working undisturbed); if `idle` and its report exists on disk, Host treats it as complete and removes it from `PendingTierRoles`; if `idle` without a report: if `ProbeCount[role] == 0`, Host sends Probe 1 (`"Status probe (do not respond via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your audit is completely finished, write your report to .scratch/deep-review/reports/<Role>.md and notify Host."`) and sets `ProbeCount[role] = 1`; if `ProbeCount[role] == 1`, Host sends Probe 2 (`"Status probe (please confirm your active status IMMEDIATELY via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your audit is completely finished, write your report to .scratch/deep-review/reports/<Role>.md and notify Host."`) and sets `ProbeCount[role] = 2`; if `ProbeCount[role] >= 2` and the reviewer remained idle with no confirmation received, Host concludes the reviewer is unrecoverable, inspects transcript/task logs, terminates it via `manage_subagents(Action="kill")`, and respawns it. Only when `PendingTierRoles` is empty does Host proceed to evaluate reports across the tier batch.
2. Host marks any issues lacking proofs, citing non-existent APIs, breaking macro flow, violating boundary contract symmetry, introducing intra-DA contradictions, or constituting invalid defects as GATED. For all roles with gated issues, Host initializes `PendingGatedRoles`, initializes per-role `ProbeCount[role] = 0`, authors `.scratch/deep-review/reports/<Role>_Gated_Issues.md` simultaneously, placing a single top-level `## Required Reviewer Action` section at the top of the file without repeating action choices per issue, demanding refinement/completion or removal without suggesting solutions, notifies them via `send_message`, and arms a 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on gated reviewers liveness", TimerCondition="any")`. Fully accepted roles are not messaged.
3. Gated reviewers respond by either:
   - Refining / Completing `<Role>.md` in-place (resolving gate failures by converting ungrounded code into abstract specifications, supplying missing boundary endpoints, or harmonizing contradicting assertions in `Verification Plan` with verified proofs). If `.scratch/deep-review/reports/<Role>_Explain.md` was authored in a prior turn of the active tier batch, reviewer MUST invalidate it (either by deleting it, or by overwriting it with empty content via `write_to_file(CodeContent="")` if native file deletion tools are unavailable) to eliminate stale defense artifacts; Host handles authoritative physical file removal upon accepting the updated report.
