...
3. **Subagent Execution & Single-Wave Invariant Summoning**:
   ...
   - If the 180s liveness check timer expires while `PendingTierRoles` is non-empty:
     ...
     4. If a reviewer's state is `idle` and its report does NOT exist on disk, send a non-pressuring status probe via `send_message` (`"Status probe (do not respond via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your audit is completely finished, write your report to .scratch/deep-review/reports/<Role>.md and notify Host."`).
     5. If non-responsive or errored, terminate and respawn that specific reviewer, resetting its pending wait state.
     6. After processing timer expiry, if `PendingTierRoles` remains non-empty, Host MUST re-arm the 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on reviewers liveness", TimerCondition="any")` and end its turn to continue waiting for reviewer responses.
...
4. **Tier Batch Gate & Reviewer Negotiation**:
   ...
   - For all roles in the tier with gated issues:
     ...
     5. If the liveness check timer expires before `PendingGatedRoles` is empty:
        ...
        - If a gated reviewer's state is `idle`, Host sends a non-pressuring status probe via `send_message` (`"Status probe (do not respond via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your gate response is completely finished, update .scratch/deep-review/reports/<Role>.md and notify Host."`).
