...
3. **Subagent Execution & Single-Wave Invariant Summoning**:
   ...
   - Host initializes a tracking set `PendingTierRoles` containing all active roles in the active tier batch, initializes a per-role probe counter `ProbeCount[role] = 0`, and arms a 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on reviewers liveness", TimerCondition="any")`.
   - Upon receiving an asynchronous reactive wakeup message from a reviewer:
     - If `.scratch/deep-review/reports/<Role>.md` exists on disk: Host removes that reviewer from `PendingTierRoles`.
     - If the report does not exist on disk but the message is an active confirmation: Host resets `ProbeCount[role] = 0`, does NOT remove the reviewer from `PendingTierRoles`, re-arms the 180s timer, and continues waiting.
     If `PendingTierRoles` is non-empty, Host re-arms the 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on reviewers liveness", TimerCondition="any")`, does NOT inspect `<Role>.md` reports on disk or advance to Step 4, and ends its turn to continue waiting for remaining subagents.
   - If the 180s liveness check timer expires while `PendingTierRoles` is non-empty:
     ...
     2. If a reviewer's state is `running`, Host MUST NOT send any message (the reviewer is actively executing tools, reading code, or running scripts). Host re-arms the 180s timer and lets the subagent continue working.
     3. If a reviewer's state is `idle` and its report `.scratch/deep-review/reports/<Role>.md` already exists on disk, Host treats that role as complete, removes it from `PendingTierRoles`, and does NOT send a message.
     4. If a reviewer's state is `idle` and its report does NOT exist on disk:
        - If `ProbeCount[role] == 0`: Host sends Probe 1 via `send_message` (`"Status probe (do not respond via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your audit is completely finished, write your report to .scratch/deep-review/reports/<Role>.md and notify Host."`), and sets `ProbeCount[role] = 1`.
        - If `ProbeCount[role] == 1`: Host sends Probe 2 via `send_message` (`"Status probe (please confirm your active status IMMEDIATELY via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your audit is completely finished, write your report to .scratch/deep-review/reports/<Role>.md and notify Host."`), and sets `ProbeCount[role] = 2`.
        - If `ProbeCount[role] >= 2` and the reviewer remained idle without sending active confirmation: Host concludes the reviewer is unrecoverable, inspects transcript/task logs for post-mortem evidence, terminates it via `manage_subagents(Action="kill")`, and respawns that specific reviewer, resetting its pending wait state.
     5. After processing timer expiry, if `PendingTierRoles` remains non-empty, Host MUST re-arm the 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on reviewers liveness", TimerCondition="any")` and end its turn to continue waiting for reviewer responses.
...
4. **Tier Batch Gate & Reviewer Negotiation**:
   ...
   - For all roles in the tier with gated issues:
     ...
     2. Host initializes `PendingGatedRoles` containing all gated roles in the active tier batch, and initializes `ProbeCount[role] = 0`.
     ...
     4. Upon receiving a reactive wakeup message from a reviewer:
        - If updated `<Role>.md` or `<Role>_Explain.md` exists on disk: Host removes that reviewer from `PendingGatedRoles`.
        - If reports are not updated but the message is an active confirmation: Host resets `ProbeCount[role] = 0`, does NOT remove reviewer from `PendingGatedRoles`, re-arms the 180s timer, and continues waiting.
        If `PendingGatedRoles` is non-empty, Host re-arms the 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on gated reviewers liveness", TimerCondition="any")`, does NOT inspect `<Role>.md` reports on disk or trigger re-gating, and ends its turn to continue waiting for remaining subagents.
     5. If the liveness check timer expires before `PendingGatedRoles` is empty:
        ...
        - If a gated reviewer's state is `running`, Host MUST NOT send any message (the reviewer is actively executing tools, investigating, or writing code). Host re-arms the 180s timer and lets it continue working.
        - If a gated reviewer's state is `idle`:
           - If `ProbeCount[role] == 0`: Host sends Probe 1 via `send_message` (`"Status probe (do not respond via send_message): Detected idle state. This probe is not an urge to rush, take your time and proceed thoroughly. If you have a running background script, check its status via manage_task. Once your gate response is completely finished, update .scratch/deep-review/reports/<Role>.md and notify Host."`), and sets `ProbeCount[role] = 1`.
           ...
           - If `ProbeCount[role] >= 2` and the gated reviewer remained idle without sending active confirmation: Host concludes the reviewer is unrecoverable, inspects transcript/task logs for post-mortem evidence, terminates it via `manage_subagents(Action="kill")`, and respawns that specific reviewer, resetting its pending wait state. Upon respawning, Host MUST immediately dispatch the gating notification message via `send_message` to the newly spawned subagent conversation ID (directing it to `.scratch/deep-review/reports/<Role>_Gated_Issues.md` and instructing it to apply the Gate Response Protocol).
        - After processing timer expiry, if `PendingGatedRoles` remains non-empty, Host MUST re-arm the 180s liveness check timer via `schedule(DurationSeconds=180, Prompt="Check on gated reviewers liveness", TimerCondition="any")` and end its turn to continue waiting for reviewer responses.
