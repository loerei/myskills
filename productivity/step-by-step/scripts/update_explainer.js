...
/**
 * Step-by-Step Visual Explainer Updater
 * Injects dynamic SVG diagrams and step progression into a persistent .scratch/explainer.html
 */
...
function parseArgs() {
  ...
  if (!options.targetDir) {
    options.targetDir = path.join(options.workspace, '.scratch');
  }
...
function buildTimelineHtml(state, currentStep) {
  return state.steps
    .map(s => {
      ...
      return `    <div class="step-pill${activeClass}"><span class="dot"></span>${s.number}. ${s.name}</div>`;
    })
    .join('\n');
}
...
function main() {
  try {
    ...
    const state = loadOrUpdateState(options.targetDir, options);
    ...
    const outFilePath = path.join(options.targetDir, 'explainer.html');
    fs.writeFileSync(outFilePath, rendered, 'utf8');

    console.log(`[+] Visual Explainer updated: ${outFilePath}`);
  } catch (err) {
